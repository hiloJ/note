    ...
    private static void threadWaitAndNotify() {
        ...
        new Thread(() -> {
            synchronized (o) {
                for (int i = 0; i < 5; i++) {
                    if (i == 3) {
                        try {
                            System.out.println("线程【" + Thread.currentThread().getName() + "】\t 即将进入等待状态");
                            o.wait();
                    ...
                    System.out.println("线程【" + Thread.currentThread().getName() + "】\t 打印当前值为：" + i);
                }
                System.out.println("线程【" + Thread.currentThread().getName() + "】\t 运行结束，即将唤醒其他等待线程");
                o.notifyAll();
            }
        }, "Thread-A").start();
        ...
        new Thread(() -> {
            synchronized (o) {
                for (int i = 0; i < 5; i++) {
                    if (i == 3) {
                        System.out.println("线程【" + Thread.currentThread().getName() + "】\t 即将唤醒其他等待线程");
                        o.notifyAll();
                        try {
                            System.out.println("线程【" + Thread.currentThread().getName() + "】\t 即将进入等待状态");
                            o.wait();
